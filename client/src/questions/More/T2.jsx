...
const T2 = () => {
    ...
    const events = [
        {
            ...
            subHeading: "Feeling sad is a normal human experience, but feeling too much sadness can cause distress and life problems. When too much sadness affects your life, you might have depression.",
            direction: "right",
        },
        {
            number:2,
            heading: "What causes Depression?",
            subHeading: "Depression is a mental health problem that comes from changes in your brain. The changes relate to how your brain produces and absorbs neurotransmitters, how it’s circuits work, inflammation, and even how it’s built (grey matter).",
            direction: "left",
        },
        {
            number:3,
    ...
    return (
        <div className="flex flex-col gap-y-3 w-full my-4 z-10">
            <span className='text-3xl font-bold bg-green-700 text-white p-7 rounded-lg'>Understanding Depression</span>
            <Circle />
            ...
            <button className='bg-green-700 text-white w-auto h-auto m-auto]'>Treatment and Recovery &#187;</button>
        </div>
    );
};
...
const EventCard = ({ number,heading, subHeading, onClick }) => {
    return (
        <div className="flex flex-col gap-y-2 border shadow-md rounded-xl p-4">
            <div className='bg-red-800 text-white rounded-full w-10 h-10'>{number}</div>
            ...
            <button onClick={onClick} className="text-blue-500 hover:text-blue-700 focus:outline-none">Read More</button>
        </div>
    );
};
...
const Modal = ({ onClose,heading }) => {
    return (
        <div className="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-80 m-auto transition-opacity delay-1000">
            <div className="bg-white p-4 rounded-md transform transition-transform delay-1000">
                <h2 className="text-xl font-bold">{heading}</h2>
                {/* <p>{subHeading}</p> */}
                <p className='text-justify m-1'>Feeling sad is a normal human experience, but feeling too much sadness can cause distress and life problems. When too much sadness affects your life, you might have depression.
...
Thoughts of death or suicide</p>
                <button onClick={onClose} className="mt-4 bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:outline-none">Close</button>
            </div>
        </div>
    );
};
